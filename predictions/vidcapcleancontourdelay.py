...
# Find the largest contour and create a mask with only the largest contour
largest_contour = max(contours, key=cv2.contourArea)
mask = np.zeros_like(binary_mask)
cv2.drawContours(mask, [largest_contour], -1, 255, -1)

# Remove smaller contours from the binary mask
cleaned_mask = cv2.bitwise_and(binary_mask, binary_mask, mask=mask)
...
# Draw the cleaned contours on the image
image_with_contour = cv2.drawContours(image_with_contour, cleaned_contours, -1, (0, 255, 0), 2)
